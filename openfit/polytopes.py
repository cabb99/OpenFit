    ...
    for i, base_set in enumerate(sets, start=1):
        for signs in itertools.product([1, -1], repeat=4):
            for permutation in itertools.permutations(
                [base_set[0] * signs[0], base_set[1] * signs[1], base_set[2] * signs[2], base_set[3] * signs[3]], 4
            ):
                vertices.add(tuple(permutation))
...
def generate_16_vertices():
    ...
    vertices = np.array(list(itertools.product([-0.5, 0.5], repeat=4)))
    return vertices
...
def generate_5_vertices():
    ...
    # Initial vertex
    vertices = (
        np.array([[0, 0, 0, 4], [s5, s5, s5, -1], [s5, -s5, -s5, -1], [-s5, s5, -s5, -1], [-s5, -s5, s5, -1]]) / 4
    )
...
def generate_rotations(n, optimize=0):
    ...
    optimize : int, optional
        Number of iterations for optimizing the distribution of rotations if n does
        not correspond to a regular polytope. If optimize is 0, no optimization is
    ...
    for i in range(n):
        ...
        # If any vertex does not match the expected pattern, print detailed information
            ...
            nearest_neighbors = [
                j
                for j in range(n)
                if i != j and np.isclose(np.linalg.norm(vertices[i] - vertices[j]), min_distance, atol=1e-5)
            ]
            expected_neighbor_list = [
                j
                for j in range(n)
                if i != j and np.isclose(np.linalg.norm(vertices[i] - vertices[j]), expected_distance, atol=1e-5)
            ]
            issue = (
                f"Vertex {i} FAILS the regularity test: {count_min_distance} nearest "
                f"neighbors at distance {expected_distance}, expected {expected_neighbors}.\n"
            )
